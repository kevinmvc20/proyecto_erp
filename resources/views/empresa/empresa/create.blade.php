...
<div class="container mx-auto p-4">
    <div class="text-xl font-bold mb-4">Crear Empresa</div>
    <form action="{{ route('empresas.store') }}" method="POST" autocomplete="off">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            <label for="gerente" class="block font-medium">Gerente:</label>
            <input type="text" id="gerente" name="gerente" placeholder="Gerente..." class="w-2/4 p-2 border border-gray-300 rounded" pattern="^[a-zA-Z0-9_áéíóúñÁÉÍÓÚÑ°\s]{0,200}" value="{{ old('gerente') }}">
            @error('gerente')
            <span class="text-red-500"> {{ $message }} </span>
            @enderror
        </div>
        <div class="mb-4">
            <label for="rubro" class="block font-medium">Rubro:</label>
            <input type="text" id="rubro" name="rubro" placeholder="Rubro..." class="w-2/4 p-2 border border-gray-300 rounded" pattern="^[a-zA-Z0-9_áéíóúñÁÉÍÓÚÑ°\s]{0,200}" value="{{ old('rubro') }}">
            @error('rubro')
            <span class="text-red-500"> {{ $message }} </span>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <a href="{{ route('empresas.index') }}" class="bg-green-500 text-white p-2 rounded">Atrás</a>
        </div>
    </form>
</div>
